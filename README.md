...
## Reliability Drift Report (Metode Tambahan)

Untuk memonitor governance model (Brier drift, hit-rate drift, calibration gap):

```bash
python -m agent.main --reliability-file tests/data/backtest_sample.csv --reliability-window 20
```

Output mencakup: `brier`, `logloss`, `hit_rate`, `calibration_gap`, serta perbandingan window sebelumnya vs window terbaru.
